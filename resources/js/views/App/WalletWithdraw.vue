<template>
    <div class="grid p-1">
        <Card class="xl:col-9 md:col-9 lg:col-9 col-12 mx-1 my-1">
            <template #title>{{ $t('app.walletWithdraw.title') }}</template>
            <template #subtitle>{{ $t('app.walletWithdraw.subtitle') }}</template>
            <template #content>
                <Stepper orientation="vertical" :activeStep="step" :linear="false">
                    <StepperPanel :header="$t('app.walletWithdraw.enterAmount')">
                        <template #content="">
                            <div class="flex flex-column">
                                <div class="w-full col-12 mt-2">
                                    <FloatLabel>
                                        <InputText id="amount" v-model="amount_form.amount" style="height: 100%; width: 100%;" />
                                        <label for="amount">{{ $t('app.walletWithdraw.amountInUSD') }}</label>
                                    </FloatLabel>
                    ...
                    <StepperPanel :header="$t('app.walletWithdraw.enterAmount')">
                        <template #content="">
                            <div class="flex flex-column">
                                <div class="w-full col-12 mt-2">
                                    <FloatLabel>
                                        <InputText id="wallet_address" v-model="wallet_form.address" style="height: 100%; width: 100%;" />
                                        <label for="wallet_address">{{ $t('app.walletWithdraw.walletAddress') }}</label>
                                    </FloatLabel>
                                    <InlineMessage class="w-full" severity="warn" v-if="b$.address.$error"> {{ b$.address.$errors[0].$message }} </InlineMessage>
                                </div>
                                <div class="w-full col-12 mt-2">
                                    <FloatLabel>
                                        <Dropdown id="currency" class="w-full" v-model="wallet_form.network" :options="depositCurrencies" optionLabel="name" optionValue="symbol" :placeholder="$t('app.walletWithdraw.selectNetwork')" />
                                        <label for="currency">{{ $t('app.walletWithdraw.network') }}</label>
                                    </FloatLabel>
                                    <small id="network-help">{{ $t('app.walletWithdraw.pleaseSelectCorrectNetwork') }}</small>
                                    <InlineMessage class="w-full" severity="warn" v-if="b$.network.$error"> {{ b$.network.$errors[0].$message }} </InlineMessage>
                                </div>
                            </div>
                    ...
                    <StepperPanel :header="$t('common.finish')">
                        <template #content="{ }">
                            <div class="flex flex-column">
                                <div class="w-full">
                                    <Message :closable="false" icon="pi pi-check-circle" class="uppercase mb-1 text-lg font-semibold" severity="success">{{ $t('app.walletWithdraw.withdrawalQueuedSuccess') }}</Message>
                                </div>
                            </div>
        ...
        <template #container="{ closeCallback }">
            <div class="flex flex-column px-5 py-5 gap-4" style="border-radius: 12px; background-image: radial-gradient(circle at left top, var(--primary-400), var(--primary-700))">
                <svg width="35" height="40" viewBox="0 0 35 40" fill="none" xmlns="http://www.w3.org/2000/svg" class="block mx-auto">
                    <path d="M25.87 18.05L23.16 17.45L25.27 20.46V29.78L32.49 23.76V13.53L29.18 14.73L25.87 18.04V18.05ZM25.27 35.49L29.18 31.58V27.67L25.27 30.98V35.49ZM20.16 17.14H20.03H20.17H20.16ZM30.1 5.19L34.89 4.81L33.08 12.33L24.1 15.67L30.08 5.2L30.1 5.19ZM5.72 14.74L2.41 13.54V23.77L9.63 29.79V20.47L11.74 17.46L9.03 18.06L5.72 14.75V14.74ZM9.63 30.98L5.72 27.67V31.58L9.63 35.49V30.98ZM4.8 5.2L10.78 15.67L1.81 12.33L0 4.81L4.79 5.19L4.8 5.2ZM24.37 21.05V34.59L22.56 37.29L20.46 39.4H14.44L12.34 37.29L10.53 34.59V21.05L12.42 18.23L17.45 26.8L22.48 18.23L24.37 21.05ZM22.85 0L22.57 0.69L17.45 13.08L12.33 0.69L12.05 0H22.85Z" fill="var(--primary-700)"/>
                    <path d="M30.69 4.21L24.37 4.81L22.57 0.69L22.86 0H26.48L30.69 4.21ZM23.75 5.67L22.66 3.08L18.05 14.24V17.14H19.7H20.03H20.16H20.2L24.1 15.7L30.11 5.19L23.75 5.67ZM4.21002 4.21L10.53 4.81L12.33 0.69L12.05 0H8.43002L4.22002 4.21H4.21002ZM21.9 17.4L20.6 18.2H14.3L13 17.4L12.4 18.2L12.42 18.23L17.45 26.8L22.48 18.23L22.5 18.2L21.9 17.4ZM4.79002 5.19L10.8 15.7L14.7 17.14H14.74H15.2H16.85V14.24L12.24 3.09L11.15 5.68L4.79002 5.2V5.19Z" fill="var(--primary-200)"/>
                </svg>
                <div class="inline-flex flex-column gap-2">
                    <InlineMessage severity="info">{{ $t('app.walletWithdraw.withdrawalChannelWallet') }}</InlineMessage>
                </div>
                <div class="inline-flex flex-column gap-2">
                    <label for="username" class="text-primary-50 font-semibold">{{ $t('app.walletWithdraw.withdrawalAmount') }}</label>
            ...
            <Fieldset>
                <template #legend>
                    <div class="flex items-center pl-2">
                        <span class="font-bold p-2">{{ $t('app.walletWithdraw.authenticateYourAccount') }}</span>
                    </div>
                </template>
                <div class="m-0 justify-center content-center items-center center w-full">
                    ...
                    <div class="row">
                        <div class="flex justify-between mt-4 self-stretch">
                            <Button :label="$t('app.walletWithdraw.resendCode')" link class="p-0"></Button>
                            <Button :label="$t('app.walletWithdraw.submitCode')" class="ml-auto" @click="validateCode"></Button>
                        </div>
                    </div>
                </div>
            </Fieldset>
...
import type { WalletWithdraw, WithdrawResponse } from '@/types/Withdrawal'
...
export default defineComponent({
    setup() {
        ...
        const v$ = useVuelidate(
            {
                amount: { 
                    ...
                    minValue: 20,
                    sufficient: helpers.withMessage(t('app.walletWithdraw.insufficientBalance'), sufficient)
                }
            }, amount_form)

        const b$ = useVuelidate({
            address : { required, minLength: minLength(32) },
            network : { required },
        }, wallet_form)
        ...
        const code_request = ref(t('app.walletWithdraw.codeRequest'))
        const code = ref('')
        ...
        const validate = async () => {
            ...
            if(!validation) {
                toast.info(t('notifications.error'), t('app.walletWithdraw.validAmountError'))
                return
            } else {
                step.value++
            }
        ...
        const next = async () => {
            displayLoader()
            const net = depositCurrencies.find(obj => obj.symbol.toUpperCase() === wallet_form.network.toUpperCase() || obj.name.toUpperCase() === wallet_form.network.toUpperCase())
            if(!net) {
                destroyLoader()
                toast.info(t('notifications.error'), t('app.walletWithdraw.validWalletError'))
                return
            }

            const data: WalletWithdraw = {
            ...
            withdrawService.submitWalletRequest(data).then((resp) => {
                destroyLoader()
                visible.value = false
                step.value++
            }).catch((error: APIError) => {
                destroyLoader()
                toast.info(t('notifications.error'), error.message ?? t('app.walletWithdraw.unknownError'))
                return
            })                
        }
        ...
        onMounted(() => {
            service.getWalletBalance().then((bal) => {
                ...
                toast.info(t('notifications.error'), t('app.walletWithdraw.balanceError'))
            })

            service.getWithdrawalChannels().then((channels) => {
                withdrawChannels.value = channels
                if(channels.wallet !== undefined) {
                    wallet_form.address = channels.wallet!.address
                    wallet_form.network = channels.wallet!.symbol ?? channels.wallet!.network
                }
            }).catch((error) => {
                console.log(error)
            })
        })
        const walletSet = computed(() => {
            return withdrawChannels.value.wallet?.address !== undefined && withdrawChannels.value.wallet?.network !== undefined && withdrawChannels.value.wallet?.symbol !== undefined
        })
        ...
        const validateCode = () => {
            ...
            withdrawService.validateWithdrawOtp(Number(amount), code.value, 'wallet')
            .then((data: WithdrawResponse) => {
                if(data.payment_id != undefined && data.twoFactor == false) {
                    toast.success(t('app.walletWithdraw.withdrawalValidated'))
                    visible.value = false
                    showOtp.value = false
                    step.value++

                    if(data.checkpoint !== undefined) {
                        notificationText.value = t('app.walletWithdraw.furtherVerification')
                        showNotify.value = true
                    }
                } else {
                    toast.error(t('app.walletWithdraw.unableToValidate'))
                }
            })
            .catch((error) => {
        ...
        return {notificationText, showNotify, validateCode, code, code_request, showOtp, depositCurrencies, amount_form, wallet_form, step, v$, b$, validate, next, previous, visible, balance, validateBank, walletSet, withdrawChannels }
    },
    methods: {
        formatCurrency
    },
