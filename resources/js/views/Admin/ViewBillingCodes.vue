        ...
        <template #container="{ closeCallback }">
            <div class="flex flex-column px-5 py-5 gap-4" style="border-radius: 12px; background-image: radial-gradient(circle at left top, var(--primary-400), var(--primary-700))">
                ...
                <div class="inline-flex flex-column gap-2">
                    <InlineMessage severity="info">Create Authorization Code</InlineMessage>
                </div>


                <div class="inline-flex flex-column gap-2">
                    <label for="username" class="text-primary-50 font-semibold">Authorization Code</label>
                    <InputText v-model="newCode.code" placeholder="eg: 6331AR4"></InputText>

                </div>  
                
                <div class="inline-flex flex-column gap-2">
                    <label for="username" class="text-primary-50 font-semibold">Authorization Type</label>
                    <Dropdown v-model="newCode.codeType" :options="codeTypes" optionValue="id" optionLabel="name" placeholder="Select a Code Type" class="w-full md:w-14rem" /> 

                </div>  

                <div class="inline-flex flex-column gap-2">
                    <label for="username" class="text-primary-50 font-semibold">Authorization User</label>
                    <Dropdown v-model="newCode.user" :options="customers"  filter optionLabel="username" optionValue="id" placeholder="Select a User" class="w-full md:w-14rem" /> 
                ...
                <div class="flex align-items-center gap-3">
                    ...
                    <Button label="Save" @click="createCode" severity="success" class="p-3 w-full text-primary-50 border-1 border-white-alpha-30 hover:bg-white-alpha-10"></Button>
...
export default defineComponent({
    ...
    setup() {
        ...
        const loadData = () => {
            network.fetch('office/codes', {}).then((response) => {
                // @ts-ignore
                const types = response.data.types.map((type): CodeType => {
                    return { 
                        id: type.id,
                        name: type.name 
                    }
                ...
                // @ts-ignore
                const users = response.data.users.map((user): Profile => {
                    return {
                        id: user.id,
        ...
        const getUsername = (id: string) => {
            if(customers.value.length > 0) {
                const user = customers.value.filter((user) => {
        ...
        const createCode = () => {
            ...
            network.push<string, {}>('office/add_code', newCode).then((response) => {
        ...
        const deleteCode = (id: string) => {
            displayLoader()
            network.push<string, {}>('office/delete_code', {id: id}).then((response) => {
                toast.success(response.data!)
                loadData()
            }).catch((error) => {
                toast.error(error.message!)
        ...
        return { getUsername, codes, customers, newCode, codeTypes, deleteCode, createCode, paymentModal, showNew, getStatus, toggleAccount, statuses, filters, isLoading, initFilters, formatCurrency, getSeverity, formatDate, clearFilter }
    },
    components: {
        Dropdown
    }
