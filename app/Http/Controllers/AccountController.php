...
class AccountController extends Controller
{
    ...
    public function update_bank(Request $request) {
        ...
        $valid = $form->validated();
        if($user != NULL) {
            BankInfo::updateOrCreate(['user_id' => $user->id],[
                'bank_name' => $valid['bank_name'],
                'account_name' => $valid['account_name'],
                'account_number' => $valid['account_number'],
                'routing_no' => $valid['routing_no'],
                'sort_code' => $valid['sort_code']
            ]);
            return $this->success([]);
        }
        return $this->failed('Unable to validate user');
    ...
    public function update_wallet(Request $request) {
        ...
        $valid = $form->validated();
        if($user != NULL) {
            WalletInfo::updateOrCreate(['user_id' => $user->id],[
