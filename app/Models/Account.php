...
class Account extends Model
{
    ...
    */
    ...
    public function getBalanceAttribute() {
        $debCheck = [
            TransactionType::PROFIT_PAY,
        ];
        $debits = $this->transactions->where('is_debit', true)
        ->where('payment_status', 'finished')
        ->sum('price_amount');

        $credits = $this->transactions->where('is_debit', false)
        ->where('payment_status', 'finished')
        ->whereNotIn('type', $debCheck)
        ->sum('price_amount');
        return floor(floatval($credits) - floatval($debits));
    }
